...
class SudokuState:
  ...
  def set_value(self, pos: (int, int), new_value: int):
    ...
    if new_value not in self.possible_values[pos_y][pos_x]:
      raise ValueError(
        f"New value {new_value} not in possible values for cell position {pos}:"
  ...
  def blocks(self) -> list:
    ...
    #
    return [
      self.values[i:i + size, j:j + size].flatten().tolist() for i in block_idx
      for j in block_idx
    ]
  ...
  def has_duplicate_non_zero_elements(self, arr: list) -> bool:
    ...
    result = bool(
      [item for item, c in Counter(arr).items() if c > 1 and item != 0])
  ...
  def __deepcopy__(self, memodict={}):
    ...
    state.possible_values = [[cell.copy() for cell in row]
...
def order_values(state: SudokuState, pos: (int, int)):
  ...
  result = sorted([
    val for val in list(state.possible_values[y][x])
    if val not in state.saturated_values and state.set_value(pos, val).solvable
  ],
                  key=lambda item: state.value_freq[item],
                  reverse=False)
  return result
...
def backtrack(state: SudokuState) -> SudokuState:
  ...
  """
    ...
    """
  solved_sudoku = SudokuState(
    np.array([[-1 for _ in range(state.n)] for _ in range(state.n)]))
  ...
  print(values)
  for val in values:
    print("Seeting value by creating a new object")
